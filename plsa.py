...
def main():
    ...
    csv_file = "data/Splitted/Test_400.csv"
    ...
    testdocs = [['test', 'text', 'special', 'words', 'scientific', 'patients'],
                ['and' 'heres' 'another' 'with' 'some' 'more' 'words' 'medical']]  # some text? the testset

    for doc in testdocs:
        PW = 0
        result=result.predict(doc)
        t = result.predict(doc)[0]
